...
data_4 <- head(data, 10)
data_4$episode_len_s <- 13
data_4$d <- 13 * diyar::episode_units$days

data_4 <- rbind(data_4, data_4)
...
l2 <- c(rep("01/04/2018", 6), rep("19/04/2018", 4))
r2 <- c(rep("16/04/2018", 6), rep("28/04/2018", 4))
...
l <- c(rep("01/04/2018", 5),"16/04/2018","19/04/2018","22/04/2018","25/04/2018","28/04/2018")
r <- c(rep("13/04/2018", 5),"16/04/2018","19/04/2018","22/04/2018","25/04/2018","28/04/2018")

l2 <- c(rep("01/04/2018", 5), rep("16/04/2018", 5))
r2 <- c(rep("13/04/2018", 5), rep("28/04/2018", 5))
e_int <- number_line(dttm(c(l,l2)), dttm(c(r,r2)))
...
test_6 <- cbind(test_6,
                uat.episodes(strata = test_6$strata,
                             date = test_6$date,
...
l <- c(rep("01/04/2018", 6),"19/04/2018","22/04/2018","25/04/2018","28/04/2018")
r <- c(rep("16/04/2018", 6),"19/04/2018","22/04/2018","25/04/2018","28/04/2018")

l2 <- c(rep("01/04/2018", 6), rep("19/04/2018", 4))
r2 <- c(rep("16/04/2018", 6), rep("28/04/2018", 4))

l3 <- c(rep("01/04/2018", 8), rep("25/04/2018", 2))
r3 <- c(rep("22/04/2018", 8), rep("28/04/2018", 2))
...
l <- c(rep("01/04/2018", 3), rep("10/04/2018", 3), rep("19/04/2018", 3), "28/04/2018")
r <- c(rep("07/04/2018", 3), rep("16/04/2018", 3), rep("25/04/2018", 3), "28/04/2018")
...
test_9a <- cbind(hospital_infections,
                 uat.episodes(sn = hospital_infections$rd_id, date = hospital_infections$date, case_length = hospital_infections$epi_len,
                              custom_sort = hospital_infections$infection, group_stats = T, to_s4 = F))

l <- c("01/04/2018", rep("07/04/2018", 6), rep("13/05/2018", 3), "31/05/2018")
r <- c("01/04/2018", rep("07/05/2018", 6), rep("25/05/2018", 3), "31/05/2018")
...
test_9b <- cbind(hospital_infections,
                 suffix(uat.episodes(sn = hospital_infections$rd_id, date = hospital_infections$date,
...
l <- c(rep("25/05/2018", 10), "31/05/2018")
r <- c(rep("01/04/2018", 10), "31/05/2018")
...
test_10a <- cbind(hospital_infections,
                  uat.episodes(sn = hospital_infections$rd_id, date = hospital_infections$date, strata = hospital_infections$patient_id,
...
l <- c(rep("01/04/2018", 3), "19/04/2018", "25/04/2018", "01/05/2018", "07/05/2018", "13/05/2018", rep("19/05/2018", 3))
r <- c(rep("13/04/2018", 3), "19/04/2018", "25/04/2018", "01/05/2018", "07/05/2018", "13/05/2018", rep("31/05/2018", 3))
e_int <- number_line(dttm(l), dttm(r))
...
test_10b <- cbind(hospital_infections,
                  uat.episodes(sn = hospital_infections$rd_id, date = hospital_infections$date,
...
l <- c("01/04/2018", rep("07/04/2018", 3), "01/04/2018", "07/04/2018", "01/04/2018", "01/04/2018", rep("19/05/2018", 2), "31/05/2018")
r <- c("13/05/2018", rep("01/05/2018", 3), "13/05/2018", "01/05/2018", "13/05/2018", "13/05/2018", rep("25/05/2018", 2), "31/05/2018")
e_int <- number_line(dttm(l), dttm(r))
...
test_11a <- cbind(
  admissions,
  uat.episodes(date = admissions$admin_period, sn=admissions$rd_id,
               case_length = list(number_line(-admissions$admin_period@.Data, 0), admissions$epi_len),
               group_stats = T, to_s4 = F))

l <- c(rep("01/01/2019", 7), rep("20/01/2019", 2))
r <- c(rep("15/01/2019", 7), rep("31/01/2019", 2))
e_int <- number_line(dttm(l), dttm(r))
...
epids <- uat.episodes(date = date(c(x1, x2, x3, x4, x5)),
                      strata = strt,
                      case_length = c,
                      recurrence_length = r,
                      to_s4=T,
...
test_that("test 'case_for_recurrence' in rolling_episodes", {
  ...
  expect_equal(v.decode(epids@case_nm), e_case_nm)
})

x <- date(c("01/01/2007","04/01/2007","12/01/2007","15/01/2007","22/01/2007"))
df <- data.frame(x=x, c=5, r=10)
epids_r <- uat.episodes(date = df$x, case_length = df$c,  recurrence_length = df$r, to_s4 = T, episode_type = "rolling")

x <- c(date("01/01/2007"), date("10/01/2007"), date("12/01/2007"), date("15/01/2007"), date("22/01/2007"))
df <- data.frame(x=x, c=5, r=10)
epids2_r <- uat.episodes(date = df$x, case_length = df$c,  recurrence_length = df$r, to_s4 = T, episode_type = "rolling")

test_that("test rolling_episodes", {
  expect_equal(epids_r@.Data, rep(1,5))
  expect_equal(v.decode(epids_r@case_nm), c("Case","Duplicate_C","Recurrent","Recurrent","Duplicate_R"))
  expect_equal(epids2_r@.Data, rep(1,5))
  expect_equal(v.decode(epids2_r@case_nm), c("Case","Recurrent","Recurrent","Duplicate_R","Recurrent"))
})

d <- seq.Date(date("01/04/2019"), date("18/04/2019"), "3 day" )
df<- data.frame(date=d)
df$r1 <- uat.episodes(d, case_length = 3, case_for_recurrence = F, to_s4 = T, episode_type = "rolling")
df$r2 <- uat.episodes(d, case_length = 3, case_for_recurrence = T, to_s4 = T, episode_type = "rolling")

test_that("test case_for_recurrence", {
  expect_equal(df$r1@.Data, rep(1,6))
  expect_equal(v.decode(df$r1@case_nm), c("Case", "Duplicate_C", rep("Recurrent", 4)))
  ...
  expect_equal(v.decode(df$r1@wind_nm$wind_nm1), c(rep("Case", 2), rep("Recurrence", 4)))
  expect_equal(v.decode(df$r2@wind_nm$wind_nm1), c(rep("Case", 2), "Recurrence", "Case for Recurrence", "Recurrence", "Case for Recurrence"))
})
...
df$c5 = c5
df$c7 = c7
df$ep1 <- uat.episodes(date = df$dt, case_length = df$c1)
df$ep2 <- uat.episodes(date = df$dt, case_length = list(df$c1, invert_number_line(df$c1)))

df$ep3 <- uat.episodes(date = df$dt, case_length = list(df$c2, index_window(df$dt)))
df$ep4 <- uat.episodes(date = df$dt, case_length = list(df$c2, invert_number_line(df$c2), index_window(df$dt)))

df$ep5 <- uat.episodes(date = df$dt, case_length = df$c3)
df$ep6 <- uat.episodes(date = df$dt, case_length = list(df$c3, invert_number_line(df$c3), index_window(df$dt)))

df$ep7 <- uat.episodes(date = df$dt, case_length = df$c3, custom_sort = df$c_sort)

df$ep8 <- uat.episodes(date = df$dt, case_length = list(df$c1, invert_number_line(df$c1)), custom_sort = df$c_sort)
df$ep9 <- uat.episodes(date = df$dt, case_length = list(df$c5, invert_number_line(df$c5)), custom_sort = df$c_sort)

#test that these two should give the same result
df$ep10 <- uat.episodes(date = df$dt, case_length = list(df$c1, invert_number_line(df$c1)), custom_sort = df$c_sort, from_last = T)
df$ep11 <- uat.episodes(date = df$dt, case_length = list(df$c5, invert_number_line(df$c5)), custom_sort = df$c_sort, from_last = T)
...
df$ep1 <- uat.episodes(date = df$x, case_length = df$ep, recurrence_length = df$rc, case_for_recurrence = T, episode_type = "rolling", rolls_max = 1)
df$ep2 <- uat.episodes(date = df$x, case_length = df$ep, recurrence_length = df$rc, case_for_recurrence = F, episode_type = "rolling", rolls_max = 1)

df$ep3 <- uat.episodes(date = df$x, case_length = list(df$ep, 0), recurrence_length = list(df$rc, 0), case_for_recurrence = T, reference_event = "first_record", episode_type = "rolling", rolls_max = 1)
df$ep4 <- uat.episodes(date = df$x, case_length = df$ep, recurrence_length = df$rc, case_for_recurrence = F, reference_event = "first_record", episode_type = "rolling", rolls_max = 1)

df$ep5 <- uat.episodes(date = df$x, case_length = list(df$ep, 0), recurrence_length = list(df$rc, 0), case_for_recurrence = T, episode_type = "rolling", rolls_max = 2)
df$ep6 <- uat.episodes(date = df$x, case_length = df$ep, recurrence_length = df$rc, case_for_recurrence = F, episode_type = "rolling", rolls_max = 2)

test_that("test wind_id and wind_nm", {
  expect_equal(df$ep1@.Data, rep(1, 5))
  expect_equal(v.decode(df$ep1@case_nm), c("Case","Recurrent", rep("Duplicate_R", 2), "Recurrent"))
  expect_equal(v.decode(df$ep1@wind_nm$wind_nm1), c("Case", rep("Recurrence", 3), "Case for Recurrence"))
  expect_equal(df$ep1@wind_id[[1]], c(rep(1,4),4))
  expect_equal(df$ep2, df$ep4)
  expect_equal(v.decode(df$ep1@case_nm), v.decode(df$ep3@case_nm))
  expect_equal(v.decode(df$ep1@wind_nm$wind_nm1), v.decode(df$ep3@wind_nm$wind_nm1))
  ...
  expect_equal(v.decode(df$ep6@case_nm), c("Case","Recurrent", rep("Duplicate_R", 2), "Recurrent"))
  expect_equal(v.decode(df$ep6@wind_nm$wind_nm1), c("Case", rep("Recurrence", 4)))
})

x <- c(1,6,7,8,10)
df1 <- data.frame(x=x, rc=0)
df1$cl <- number_line(9,10)
df1$ep1 <- uat.episodes(date = df1$x, case_length = df1$cl, recurrence_length = df1$rc, episode_type = "rolling", skip_if_b4_lengths = T)
df1$ep2 <- uat.episodes(date = df1$x, case_length = df1$cl, recurrence_length = df1$rc, episode_type = "rolling", skip_if_b4_lengths = F)

x <- seq(1,20,3)
df2 <- data.frame(x=x, rc=0)
df2$ep <- number_line(6,6)
df2$ep3 <- uat.episodes(date = df2$x, case_length = df2$ep, recurrence_length = df2$rc, episode_type = "rolling", skip_if_b4_lengths = T)
df2$ep4 <- uat.episodes(date = df2$x, case_length = df2$ep, recurrence_length = df2$rc, episode_type = "rolling", skip_if_b4_lengths = F)

test_that("test cut-off ranges", {
  expect_equal(df1$ep1@.Data, c(1,2,3,4,1))
  expect_equal(df1$ep1@wind_id[[1]], df1$ep1@.Data)
  expect_equal(v.decode(df1$ep1@case_nm), c("Case","Skipped","Skipped","Skipped","Duplicate_C"))
  expect_equal(v.decode(df1$ep1@wind_nm$wind_nm1), c("Case","Skipped","Skipped","Skipped","Case"))

  expect_equal(df1$ep1@.Data, df1$ep2@.Data)
  expect_equal(df1$ep1@wind_id[[1]], df1$ep1@wind_id[[1]])
  expect_equal(v.decode(df1$ep2@case_nm), c(rep("Case",3), "Case","Duplicate_C"))
  expect_equal(v.decode(df1$ep2@wind_nm$wind_nm1), c(rep("Case",3), "Case", "Case"))

  expect_equal(df2$ep3@.Data, c(1,2,1,4,5,4,7))
  expect_equal(df2$ep3@.Data, c(1,2,1,4,5,4,7))
  expect_equal(v.decode(df2$ep3@wind_nm$wind_nm1), c("Case","Skipped", "Case", "Case", "Skipped", "Case", "Case"))
  expect_equal(v.decode(df2$ep3@case_nm), c("Case","Skipped", "Duplicate_C", "Case", "Skipped", "Duplicate_C", "Case"))

  expect_equal(df2$ep4@.Data, c(1,2,1,2,5,6,5))
  expect_equal(df2$ep4@.Data, df2$ep4@wind_id[[1]])
  expect_equal(v.decode(df2$ep4@wind_nm$wind_nm1), c(rep("Case",5), "Case", "Case"))
  expect_equal(v.decode(df2$ep4@case_nm), c("Case","Case", "Duplicate_C","Duplicate_C", "Case", "Case", "Duplicate_C"))
})

ds <- data.frame(x =  seq(1,25,5), c1 =10, s = c(2,2,1,1,1))
...
ds$c4 <- 0
ds$ep_1 <- uat.episodes(date = ds$x, case_length = ds$c1, skip_if_b4_lengths = T)
ds$ep_2 <- uat.episodes(date = ds$x, case_length = ds$c1, custom_sort = ds$s)
ds$ep_3 <- uat.episodes(date = ds$x, case_length = ds$c2, custom_sort = ds$s, skip_if_b4_lengths = T)
ds$ep_4 <- uat.episodes(date = ds$x, case_length = ds$c2, skip_if_b4_lengths = T)
ds$ep_5 <- uat.episodes(date = ds$x, case_length = ds$c2, episode_type = "rolling", skip_if_b4_lengths = T)
ds$ep_6 <- uat.episodes(date = ds$x, case_length = ds$c4, recurrence_length = ds$c2, episode_type = "rolling", skip_if_b4_lengths = T)
ds$ep_7 <- uat.episodes(date = ds$x, case_length = ds$c4, recurrence_length = ds$c3, episode_type = "rolling")
ds$ep_8 <- uat.episodes(date = ds$x, case_length = ds$c2, recurrence_length = ds$c3, episode_type = "rolling", skip_if_b4_lengths = T)


ds$c5 <- number_line(-7,-10)
ds$c6 <- number_line(-10,-0)
ds$c7 <- number_line(-10, 10)
ds$ep_9 <- uat.episodes(date = ds$x, case_length = ds$c2, custom_sort = ds$s, from_last = T)
ds$ep_10 <- uat.episodes(date = ds$rx, case_length = ds$c2, custom_sort = ds$s)
ds$ep_11 <- uat.episodes(date = ds$x, case_length = ds$c5, custom_sort = ds$s, from_last = T)
ds$ep_11.2 <- uat.episodes(date = ds$x, case_length = ds$c2, custom_sort = ds$s, skip_if_b4_lengths = T)

ds$ep_12 <- uat.episodes(date = ds$x, case_length = list(ds$c5, index_window(ds$x)), custom_sort = ds$s, skip_if_b4_lengths = T)
ds$ep_13 <- uat.episodes(date = ds$x, case_length = ds$c6, custom_sort = ds$s)

ds$ep_14 <- uat.episodes(date = ds$x, case_length = ds$c7, custom_sort = ds$s)
ds$ep_15 <- uat.episodes(date = ds$x, case_length = list(ds$c7, invert_number_line(ds$c7)), custom_sort = ds$s)
ds$ep_16 <- uat.episodes(date = ds$x, case_length = ds$c7, custom_sort = ds$s, from_last = T)

test_that("test concepts in event grouping", {
  # Simple fixed episode grouping
  expect_equal(ds$ep_1@.Data, c(1,1,1,4,4))
  expect_equal(v.decode(ds$ep_1@case_nm), c("Case", "Duplicate_C","Duplicate_C", "Case","Duplicate_C"))
  # Fixed episode grouping with custom sort
  expect_equal(ds$ep_2@.Data, c(1,1,3,3,3))
  expect_equal(v.decode(ds$ep_2@case_nm), c("Case", "Duplicate_C","Case", "Duplicate_C","Duplicate_C"))
  # Fixed episode grouping ranges for length
  expect_equal(ds$ep_3@.Data, c(1,2,3,4,3))
  expect_equal(v.decode(ds$ep_3@case_nm), c("Case", "Skipped","Case", "Skipped","Duplicate_C"))
  expect_equal(ds$ep_4@.Data, c(1,2,1,4,5))
  expect_equal(v.decode(ds$ep_4@case_nm), c("Case", "Skipped","Duplicate_C","Case", "Skipped"))
  # Rolling episode grouping ranges for length
  expect_equal(ds$ep_5@.Data, c(1,2,1,4,1))
  expect_equal(v.decode(ds$ep_5@case_nm), c("Case", "Skipped","Duplicate_C","Skipped", "Recurrent"))
  expect_equal(ds$ep_6@.Data, c(1,2,1,4,1))
  expect_equal(v.decode(ds$ep_6@case_nm), c("Case", "Skipped","Recurrent","Skipped", "Recurrent"))
  expect_equal(ds$ep_7@.Data, c(1,1,1,1,1))
  expect_equal(v.decode(ds$ep_7@case_nm), c("Case", "Recurrent","Duplicate_R", "Recurrent", "Duplicate_R"))
  expect_equal(ds$ep_8@.Data, c(1,2,1,1,1))
  expect_equal(v.decode(ds$ep_8@case_nm), c("Case", "Skipped","Duplicate_C", "Recurrent", "Duplicate_R"))
  # from_last - use ep_10 approach later
  expect_equal(ds$ep_9@.Data, ds$ep_10@.Data)
  expect_equal(v.decode(ds$ep_9@case_nm), v.decode(ds$ep_10@case_nm))
  expect_equal(ds$ep_3, ds$ep_11.2)
  expect_equal(ds$ep_11@.Data, c(1,2,3,4,5))
  expect_equal(v.decode(ds$ep_11@case_nm), c("Case", "Case","Case","Case", "Case"))

  # Neg lengths
  expect_equal(ds$ep_12@.Data, c(3,2,3,4,5))
  expect_equal(v.decode(ds$ep_12@case_nm), c("Duplicate_C", "Skipped","Case", "Case", "Case"))
  expect_equal(ds$ep_13@.Data, c(3,3,3,4,5))
  expect_equal(v.decode(ds$ep_13@case_nm), c("Duplicate_C", "Duplicate_C","Case", "Case", "Case"))

  # Case level bi_direction
  expect_equal(ds$ep_15, ds$ep_14)
  expect_equal(ds$ep_16@.Data, c(2,2,5,5,5))
  expect_equal(v.decode(ds$ep_16@case_nm), c("Duplicate_C", "Case", "Duplicate_C","Duplicate_C", "Case"))

})

x =  seq(1,25,5)
y =  seq(2,22, 5)
nl = number_line(l=c(x,y),r=c(x+3, y+1))

ds <- data.frame(c1 = 10,  s = rep(c(2,2,1,1,1),2))
ds$x <- nl
...
ds$c3 <- number_line(0,10)
ds$c4 <- 0
ds$ep_1 <- uat.episodes(date = ds$x, case_length = list(ds$c1, index_window(ds$x)))
ds$ep_2 <- uat.episodes(date = ds$x, case_length = list(ds$c1, index_window(ds$x)), custom_sort = ds$s)
ds$ep_3 <- uat.episodes(date = ds$x, case_length = list(ds$c2, index_window(ds$x)), custom_sort = ds$s, skip_if_b4_lengths = T)
ds$ep_3.5 <- uat.episodes(date = ds$x, case_length = ds$c2, custom_sort = ds$s, skip_if_b4_lengths = T)

ds$ep_4 <- uat.episodes(date = ds$x, case_length = list(ds$c2, index_window(ds$x)), skip_if_b4_lengths = T)
ds$ep_5 <- uat.episodes(date = ds$x, case_length = list(ds$c2, index_window(ds$x)), episode_type = "rolling", skip_if_b4_lengths = T)
ds$ep_5.5 <- uat.episodes(date = ds$x, case_length = ds$c2, episode_type = "rolling", skip_if_b4_lengths = T)
ds$ep_6 <- uat.episodes(date =ds$x, case_length = list(ds$c4, index_window(ds$x)), recurrence_length = list(ds$c2, index_window(ds$x)), episode_type = "rolling", skip_if_b4_lengths = T)
ds$ep_7 <- uat.episodes(date =ds$x, case_length = list(ds$c4, index_window(ds$x)), recurrence_length = list(ds$c3, index_window(ds$x)), episode_type = "rolling")
ds$ep_8 <- uat.episodes(date = ds$x, case_length = list(ds$c2, index_window(ds$x)), recurrence_length = list(ds$c3, index_window(ds$x)), episode_type = "rolling", skip_if_b4_lengths = T)

ds$c5 <- number_line(-7,-10)
ds$c6 <- number_line(-10,-0)
ds$c7 <- number_line(-10, 10)

ds$ep_9 <- uat.episodes(date = ds$x, case_length = list(ds$c2, index_window(ds$x, from_last = T)), custom_sort = ds$s, from_last = T, skip_if_b4_lengths = T)
ds$ep_9.2 <- uat.episodes(date = ds$x, case_length = ds$c2, custom_sort = ds$s, from_last = T, skip_if_b4_lengths = T)

ds$ep_11 <- uat.episodes(date = ds$x, case_length = list(ds$c5, index_window(ds$x)), custom_sort = ds$s, from_last = T)
ds$ep_11.2 <- uat.episodes(date = ds$x, case_length = list(ds$c2, index_window(ds$x)), custom_sort = ds$s, skip_if_b4_lengths = T)

ds$ep_12 <- uat.episodes(date = ds$x, case_length = list(ds$c5, index_window(ds$x)), custom_sort = ds$s)
ds$ep_13 <- uat.episodes(date = ds$x, case_length = list(ds$c6, index_window(ds$x)), custom_sort = ds$s)
...
test_that("test concepts in interval grouping", {
  ...
  expect_equal(v.decode(ds$ep_1@case_nm), c("Case", rep("Duplicate_C",5), "Case", rep("Duplicate_C",3)))
  # Fixed episode grouping with custom sort
  expect_equal(ds$ep_2@.Data, c(rep(1,4), rep(5,6)))
  expect_equal(v.decode(ds$ep_2@case_nm), c("Case", rep("Duplicate_C",3), "Case", rep("Duplicate_C",5)))
  # Fixed episode grouping ranges for length
  # ep3 and ep3.5 include_inital_period
  expect_equal(ds$ep_3@.Data, c(1,1,3,4,5,5,7,8,5,5))
  expect_equal(v.decode(ds$ep_3@case_nm), c("Case","Duplicate_C","Skipped","Skipped","Case","Duplicate_C", "Skipped","Skipped", "Duplicate_C", "Duplicate_C"))
  expect_equal(ds$ep_3.5@.Data, c(1,2,3,4,5,6,7,8,5,5))
  expect_equal(v.decode(ds$ep_3.5@case_nm), c("Case","Case","Skipped","Skipped","Case","Case", "Skipped","Skipped", "Duplicate_C", "Duplicate_C"))
  expect_equal(ds$ep_4@.Data, c(1,1,3,4,1,1,7,7,9,10))
  expect_equal(v.decode(ds$ep_4@case_nm), c("Case","Duplicate_C","Skipped","Skipped","Duplicate_C","Duplicate_C", "Case","Duplicate_C", "Skipped", "Skipped"))

  # Rolling episode grouping ranges for length
  expect_equal(ds$ep_5@.Data, c(1,1,3,4,1,1,7,8,1,1))
  expect_equal(v.decode(ds$ep_5@case_nm), c("Case","Duplicate_C","Skipped","Skipped","Duplicate_C","Duplicate_C", "Skipped","Skipped", "Recurrent", "Duplicate_R"))
  expect_equal(ds$ep_5.5@.Data, c(1,2,3,4,1,1,7,8,1,1))
  expect_equal(v.decode(ds$ep_5.5@case_nm), c("Case","Case","Skipped","Skipped","Duplicate_C","Duplicate_C", "Skipped","Skipped", "Recurrent", "Duplicate_R"))
  expect_equal(ds$ep_6@.Data, c(1,1,3,4,1,1,7,8,1,1))
  expect_equal(v.decode(ds$ep_6@case_nm), c("Case","Duplicate_C","Skipped","Skipped","Recurrent","Duplicate_R", "Skipped","Skipped", "Recurrent", "Duplicate_R"))
  expect_equal(ds$ep_7@.Data, rep(1,10))
  expect_equal(v.decode(ds$ep_7@case_nm), c("Case","Duplicate_C","Recurrent",rep("Duplicate_R", 3),"Recurrent",rep("Duplicate_R", 3)))
  expect_equal(ds$ep_8@.Data, c(1,1,3,4,rep(1,6)))
  expect_equal(v.decode(ds$ep_8@case_nm), c("Case","Duplicate_C","Skipped","Skipped","Duplicate_C","Duplicate_C", "Recurrent","Duplicate_R", "Duplicate_R", "Duplicate_R"))

  # from_last
  expect_equal(ds$ep_9@.Data, c(4,2,4,4,10,10,10,8,10,10))
  expect_equal(v.decode(ds$ep_9@case_nm), c("Duplicate_C","Skipped","Duplicate_C","Case","Duplicate_C","Duplicate_C", "Duplicate_C","Skipped", "Duplicate_C", "Case"))
  expect_equal(ds$ep_9.2@.Data, c(4,2,3,4,10,10,10,8,9,10))
  expect_equal(v.decode(ds$ep_9.2@case_nm), c("Duplicate_C","Skipped","Skipped","Case","Duplicate_C","Duplicate_C", "Duplicate_C","Skipped", "Skipped", "Case"))

  expect_equal(ds$ep_11@.Data, c(2,2,4,4,6,6,8,8,10,10))
  expect_equal(v.decode(ds$ep_11@case_nm), c("Duplicate_C","Case","Duplicate_C","Case","Duplicate_C","Case", "Duplicate_C","Case", "Duplicate_C", "Case"))
  expect_equal(ds$ep_11.2@.Data, c(1,1,3,4,5,5,7,8,5,5))
  expect_equal(v.decode(ds$ep_11.2@case_nm), c("Case","Duplicate_C","Skipped","Skipped", "Case", "Duplicate_C", "Skipped", "Skipped", "Duplicate_C", "Duplicate_C"))
  expect_equal(ds$ep_3, ds$ep_11.2)

  # Neg lengths
  expect_equal(ds$ep_12@.Data, c(5,2,5,5,5,5,7,7,9,9))
  expect_equal(v.decode(ds$ep_12@case_nm), c("Duplicate_C","Case","Duplicate_C","Duplicate_C", "Case", "Duplicate_C", "Case", "Duplicate_C", "Case", "Duplicate_C"))
  expect_equal(ds$ep_12@.Data, c(5,2,5,5,5,5,7,7,9,9))
  expect_equal(v.decode(ds$ep_12@case_nm), c("Duplicate_C","Case","Duplicate_C","Duplicate_C", "Case", "Duplicate_C", "Case", "Duplicate_C", "Case", "Duplicate_C"))
  ...
  expect_equal(v.decode(ds$ep_14@case_nm), c("Duplicate_C","Case","Duplicate_C","Duplicate_C", "Case", rep("Duplicate_C",5)))
  expect_equal(ds$ep_15, ds$ep_14)

  expect_equal(ds$ep_16@.Data, c(rep(rep(4,4)),rep(10, 6)))
  expect_equal(v.decode(ds$ep_16@case_nm), c(rep("Duplicate_C",3), "Case",rep("Duplicate_C",5), "Case"))
...
ep <- uat.episodes(
  date = dt, custom_sort = c(1:3), episodes_max = 1, case_length = 2,
  reference_event = "last_event", batched = "yes")

test_that("xxx", {
  expect_equal(ep@.Data, c(1,1,1))
  expect_equal(v.decode(ep@case_nm), c("Case", "Case", "Duplicate_C"))
})
...
test_that("batched + skip_if_b4_lengths", {
  ...
  expect_equal(v.decode(ep@case_nm), c("Case", "Case", "Skipped", "Skipped", "Duplicate_C"))
  expect_equal(ep2@.Data, c(1,1,3,3,1))
  expect_equal(v.decode(ep2@case_nm), c("Case", "Case", "Case", "Case", "Duplicate_C"))
  expect_equal(ep, ep3)
})
...
mth1 <- uat.episodes(date = dates, case_length = 4)

# Track fixed episodes from periods that are 5 days long
mth2 <- uat.episodes(date = periods, case_length = 0)

# Track rolling episodes from events using a 5 day cut-off
mth3 <- uat.episodes(date = dates, case_length = 4, episode_type = "rolling")

# Track rolling episode from periods that are 5 days long
mth4 <- uat.episodes(date = periods, case_length = 0, episode_type = "rolling")
...
test_that("test interchangeable use of interval grouping and event grouping ", {
  ...
  expect_equal(v.decode(mth1@case_nm), c("Case", "Duplicate_C", "Case", "Duplicate_C", "Case"))
  expect_equal(v.decode(mth1@wind_nm$wind_nm1), c("Case", "Case", "Case", "Case", "Case"))
  expect_equal(mth3@.Data, c(1,1,1,1,1))
  expect_equal(mth3@wind_id[[1]], c(1,1,2,3,4))
  expect_equal(v.decode(mth3@case_nm), c("Case", "Duplicate_C", "Recurrent", "Recurrent", "Recurrent"))
  expect_equal(v.decode(mth3@wind_nm$wind_nm1), c("Case", "Case", "Recurrence", "Recurrence", "Recurrence"))
